...
def plot_arrow(x, y, yaw, length=1.0, width=0.5, fc="r", ec="k"):
    ...
    Args:
        x,y,yaw : 坐标
        length : 长度. Defaults to 1.0.
        width : 宽度. Defaults to 0.5.
        fc : 箭头颜色 . Defaults to "r".
        ec : 边框颜色. Defaults to "k".
    """
...
def main():
    ...
    if show_animation:  # pragma: no cover
        plt.cla()
        plt.title("final course " + str(modes))
        for i in range(0,len(xs),5):
            plt.plot(xs[0:i], ys[0:i],'k')
            # plotting
            plot_arrow(start_x, start_y, start_yaw,fc="b")
            plot_arrow(end_x, end_y, end_yaw,fc="g")
